...
pub fn format_safe_diff(changes: &[SymbolChange], budget_limit: usize) -> String {
    ...
    if total > budget_limit {
        out.push_str(&format!(
            "[WARNING] Diff too large ({} changes). Showing high-risk summary only.\n\n",
            total
        ));
        let removed: Vec<_> = changes
            .iter()
            .filter(|c| matches!(c, SymbolChange::Removed { .. }))
            .collect();
        let moved: Vec<_> = changes
            .iter()
            .filter(|c| matches!(c, SymbolChange::CommunityChanged { .. }))
            .collect();
        let added: Vec<_> = changes
            .iter()
            .filter(|c| matches!(c, SymbolChange::Added { .. }))
            .collect();
        out.push_str(&format!(
            "### Totals:\n- Removed: {}\n- Community moves: {}\n- Added: {}\n\n",
            removed.len(),
        ...
        if !moved.is_empty() {
            ...
            for ch in moved.iter().take(10) {
                if let SymbolChange::CommunityChanged {
                    id,
                    old_community,
                    new_community,
                    ..
